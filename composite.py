...
def sample_from_all_combinations(all_items, num_samples):
    """Randomly sample from the combination space"""
    """This function is taken from: https://stackoverflow.com/questions/22229796/choose-at-random-from-combinations"""
...
class Topology:
    def __init__(self, filename):
        ...
        # file_name = "randomdeg4.pkl"
        open_file = open(file_name, "rb")
        treefuns = pickle.load(open_file)
        open_file.close()
        for u, deg in self.topology.in_degree():
            if deg and self.topology.out_degree(u):
                self.nodemap[u] = Node(u, treefuns, self.topology)
    ...
    def find_decoupling(self):
        ...
        for i in range(sz-1, -1, -1):
            ...
            for j in range(len(set2keys), 0, -1):
                edgelim = 200000
                ncr = comb(len(set2keys), j)
                if(ncr > edgelim):
                    edges = sample_from_all_combinations(set2keys, edgelim)
                else:
